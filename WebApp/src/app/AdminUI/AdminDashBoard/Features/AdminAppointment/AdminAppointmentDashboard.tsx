...
<button
  ...
  className="w-full sm:w-auto bg-[#113F67] text-white px-5 py-2.5 rounded-xl hover:bg-[#0d2f4d] transition shadow-md hover:shadow-lg font-semibold"
>
  Scan QR Code
</button>
...
{/* UNAVAILABLE PANEL */}
<div className="mb-6 rounded-2xl border border-[#113F67]/10 bg-white shadow-md p-4 sm:p-5">
...
<p className="text-sm text-gray-600 mt-3 mb-4">
  Block booking dates and prepare for notifications.
</p>
...
<div className="grid gap-3 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
  <input
    ...
    value={unavailableDate}
    onChange={(e) => setUnavailableDate(e.target.value)}
    className="rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-[#113F67]/30 focus:border-[#113F67]"
  />
  <input
    type="text"
    value={unavailableReason}
    onChange={(e) => setUnavailableReason(e.target.value)}
    placeholder="Reason (e.g. lab maintenance)"
    className="rounded-lg border border-gray-300 px-3 py-2 md:col-span-2 focus:ring-2 focus:ring-[#113F67]/30 focus:border-[#113F67]"
  />
</div>
...
<div className="mt-4">
  <button
    onClick={handleSetUnavailableDate}
    disabled={savingUnavailable}
    className="w-full sm:w-auto bg-[#113F67] text-white px-4 py-2 rounded-lg hover:bg-[#0d2f4d] transition shadow"
  >
    {savingUnavailable ? 'Saving...' : 'Mark Unavailable'}
  </button>
</div>
...
<div className="mt-4 space-y-2">
  {unavailableDates.map((item) => (
    <div key={item.unavailable_id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 bg-gray-50 p-3 rounded-lg border">
    ...
    {/* TABS */}
    <div className="flex gap-3 mb-6 border-b border-gray-200 overflow-x-auto no-scrollbar">
      {tabs.map((tab) => (
        <button
          key={tab.key}
          onClick={() => setActiveTab(tab.key)}
          className={`relative px-5 py-3 font-semibold whitespace-nowrap transition ${
            activeTab === tab.key
              ? 'text-[#113F67]'
        ...
        >
          ...
          {activeTab === tab.key && (
            <span className="absolute left-0 bottom-0 w-full h-[2px] bg-[#113F67]" />
          )}
      ...
      <div className="space-y-4">
        {appointments.map((appointment) => (
          <div key={appointment.appointment_id}
            className="bg-white p-4 sm:p-6 rounded-2xl shadow-md hover:shadow-xl transition border border-gray-100"
          >
            <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <h3 className="font-semibold text-[#113F67]">
                    {appointment.student_id}
                  </h3>
  
                  <span className="px-3 py-1 text-xs rounded-full bg-[#113F67]/10 text-[#113F67] font-semibold">
                    {appointment.status.toUpperCase()}
                  </span>
                </div>
  
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600">
                  <div><b>Department:</b> {appointment.department}</div>
                  <div><b>Date:</b> {format(new Date(appointment.date), 'MMM dd, yyyy hh:mm a')}</div>
                  <div className="col-span-1 sm:col-span-2"><b>Purpose:</b> {appointment.purpose}</div>
                </div>
              </div>
              ...
              {activeTab === 'pending' && (
               ...
               <button
                 onClick={() => openModal(appointment, 'approve')}
                 className="w-full sm:w-auto bg-[#113F67] text-white px-4 py-2 rounded-lg hover:bg-[#0d2f4d] transition font-semibold text-sm"
               >
                 Approve
               </button>
               ...
               <button
                 onClick={() => openModal(appointment, 'deny')}
                 className="w-full sm:w-auto border border-red-500 text-red-600 px-4 py-2 rounded-lg hover:bg-red-50 transition font-semibold text-sm"
               >
                 Deny
               </button>
             </div>
    ...
    {showModal && (
      <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 px-3">
        <div className="bg-white rounded-2xl shadow-2xl p-4 sm:p-6 w-full max-w-md">
          ...
          <h2 className="text-lg sm:text-xl font-bold text-[#113F67] mb-4">
            {modalType === 'scan' && 'Scan QR Code'}
            {modalType === 'approve' && 'Approve Appointment'}
            {modalType === 'deny' && 'Deny Appointment'}
          </h2>
          ...
          {modalType === 'scan' ? (
            <div className="space-y-4">
              ...
              <button
                onClick={() => startCamera()}
                className="w-full bg-[#113F67] text-white py-3 rounded-lg hover:bg-[#0d2f4d]"
              >
                Start Scanner
              </button>
              ...
              <input
                ...
                value={qrInput}
                onChange={(e) => setQrInput(e.target.value)}
                className="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#113F67]/30"
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <button
                  onClick={handleScanQR}
                  className="flex-1 bg-[#113F67] text-white py-2 rounded-lg"
                >
                  Verify
                </button>
                <button
            ...
            <div>
              ...
              <textarea
                value={remarks}
                onChange={(e) => setRemarks(e.target.value)}
                className="w-full border rounded-lg p-2 mb-4 focus:ring-2 focus:ring-[#113F67]/30"
                placeholder="Remarks..."
              />
